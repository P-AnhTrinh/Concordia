...
public class Client extends Thread {
     ...
     public void sendTransactions()
     {
         ...
         {  
            while( objNetwork.getInBufferStatus().equals("full") ) {/* Alternatively, busy-wait until the network input buffer is available */
                //System.out.println("in buffer full, wait");
                Thread.yield();
               // System.out.println("client full loop");
            }
         ...
         objNetwork.setClientConnectionStatus("idle"); //if finish sending, set connection status idle
    }
     ...
     public void receiveTransactions(Transactions transact)
     {
         ...
         {     
        	 while( objNetwork.getOutBufferStatus().equals("empty")){
        	     Thread.yield();
        	     //System.out.println("client empty loop");
	...
	/* Implement the code for the run method */
        if (getClientOperation().equals("sending")) {
            ...
            //System.out.println("CLIENT status: "+objNetwork.getClientConnectionStatus());
           // System.out.println("SERVER status: "+objNetwork.getServerConnectionStatus());
        }
        else{
           // System.out.println("Client thread disconnected");
            System.exit(0);
