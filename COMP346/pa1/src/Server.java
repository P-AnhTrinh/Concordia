...
public class Server extends Thread{
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
         ...
         { 
        	 while( (objNetwork.getInBufferStatus().equals("empty"))){ //client is disconnected before this loop => infinite loop (deadlock), need to put a condition
        	     if (objNetwork.getClientConnectionStatus().equals("idle") ||objNetwork.getClientConnectionStatus().equals("disconnected")){ //if server done processing all transactions of client, break the loop
        	         break;
                 }
        	     //System.out.println("In buffer is empty");
        	     Thread.yield(); //waiting
        	     //System.out.println("Server class empty loop, check again in buffer");
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
        		 ...
        		 while( (objNetwork.getOutBufferStatus().equals("full"))){
        		     //System.out.println("SERVER out buffer full");
        		     Thread.yield();
        		     //System.out.println("SERVER resending into out buffer");
    ...
    public void run() {
        ...
        if (objNetwork.getClientConnectionStatus().equals("idle")){
            objNetwork.setServerConnectionStatus("idle");
        } //if client status is idle, set the connection status to be idle

        else if (objNetwork.getClientConnectionStatus().equals("disconnected")){
            objNetwork.disconnect(objNetwork.getServerIP());
        } //if client disconnected, we will disconnect the server
